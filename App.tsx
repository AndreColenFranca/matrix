...
const App: React.FC = () => {
  ...
  const sendToWhatsApp = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(response.status === 401 ? "Token Inválido (401)" : response.status === 503 ? "WhatsApp Desconectado (503)" : "Erro ao enviar");
      }
    } catch (err: any) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 pb-20">
      {/* Toast Notification */}
      <div className={`fixed bottom-8 left-1/2 -translate-x-1/2 z-[100] transition-all duration-300 transform ${toast.visible ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0 pointer-events-none'}`}>
        <div className={`px-6 py-3 rounded-full shadow-2xl border flex items-center gap-3 font-semibold text-sm ${
          ...
          <button onClick={() => setToast(prev => ({ ...prev, visible: false }))} className="hover:opacity-70">
      ...
      <header className="mb-10 text-center relative">
        <div className="absolute right-0 top-0 hidden md:flex items-center gap-2">
          <span className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase border transition-colors ${
            ...
            connectionStatus === 'offline' ? 'bg-red-50 text-red-600 border-red-100' : 'bg-slate-50 text-slate-400 border-slate-100'
          }`}>
            <span className={`w-1.5 h-1.5 rounded-full ${connectionStatus === 'online' ? 'bg-emerald-500 animate-pulse' : connectionStatus === 'offline' ? 'bg-red-500' : 'bg-slate-300'}`}></span>
            {connectionStatus === 'online' ? 'Conectado' : connectionStatus === 'offline' ? 'Desconectado' : 'Pronto'}
          </span>
        </div>
      ...
      <div className="flex flex-col gap-8">
        <div className="flex flex-col md:flex-row gap-4 items-center justify-center sticky top-4 z-40 bg-white/80 backdrop-blur-md p-4 rounded-2xl shadow-lg border border-slate-100">
          ...
          <div className="flex gap-2">
            ...
            >
              {sending ? <i className="fas fa-spinner fa-spin"></i> : <i className="fab fa-whatsapp text-lg"></i>}
              Enviar
            </button>
            <button onClick={() => setIsSettingsOpen(true)} className={`p-3 rounded-xl transition-all ${isSettingsOpen ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-indigo-600 hover:bg-indigo-50'}`} title="Configurações">
          ...
          <div className="bg-amber-50 border border-amber-100 p-6 rounded-2xl animate-in fade-in slide-in-from-top-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
              <div className="space-y-2">
                ...
                <p className="text-slate-600">"Finalizar relatório do cliente que vence em 1 hora"</p>
              </div>
              <div className="space-y-2">
                <p className="font-bold text-blue-600 uppercase">📅 SCHEDULE (Agendar)</p>
                <p className="text-slate-600">"Planejar as metas de marketing para o próximo semestre"</p>
              </div>
              <div className="space-y-2">
                <p className="font-bold text-amber-600 uppercase">👤 DELEGATE (Delegar)</p>
                <p className="text-slate-600 italic">"Comprar cartuchos de tinta urgente para a reunião de hoje"</p>
              </div>
              <div className="space-y-2">
                <p className="font-bold text-slate-500 uppercase">🗑️ ELIMINATE (Eliminar)</p>
                <p className="text-slate-600">"Ficar olhando o feed do Instagram sem objetivo"</p>
              </div>
            </div>
            <p className="mt-4 text-[10px] text-amber-700 italic border-t border-amber-200 pt-2">
              Dica: Seja específico. Em vez de "Comprar coisas", use "Comprar suprimentos de escritório urgentes".
            </p>
          </div>
        )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden animate-in fade-in zoom-in duration-200 border border-slate-200">
            <div className="px-6 py-4 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
              <h2 className="font-bold text-slate-800 flex items-center gap-2"><i className="fas fa-plug text-indigo-500"></i> Configuração UAZAPI</h2>
              <button onClick={() => setIsSettingsOpen(false)} className="text-slate-400 hover:text-slate-600 p-1"><i className="fas fa-times"></i></button>
            </div>
            <div className="p-6 space-y-5">
              {apiLastError === 503 && (
                <div className="p-4 bg-red-50 border border-red-200 rounded-xl space-y-3">
                  <div className="flex items-start gap-3">
                    <div className="bg-red-100 p-2 rounded-lg text-red-600 mt-1"><i className="fas fa-mobile-screen-button"></i></div>
                    <div>
                      <h3 className="font-bold text-red-800 text-sm">WhatsApp Desconectado (503)</h3>
                      <p className="text-xs text-red-700 mt-1">Sua instância perdeu a conexão. Siga os passos no painel:</p>
                    </div>
                  </div>
                  <ol className="text-xs text-red-800 space-y-2 ml-10 list-decimal font-medium">
                    <li>Acesse <b>uazapi.com</b></li>
                    <li>Clique em <b>Excluir Sessão</b>.</li>
                    <li>Clique em <b>Gerar QR Code</b> e escaneie.</li>
                  </ol>
                  <a href="https://uazapi.com" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg text-xs font-bold transition-all">Abrir Painel <i className="fas fa-external-link-alt text-[10px]"></i></a>
                </div>
              )}
      ...
      <footer className="mt-12 text-center text-slate-400 text-sm">
        <p>Inteligência Artificial por <b>Gemini 3 Flash</b></p>
        <p>Inteligência Artificial por <b>Gemini 4 Flash</b></p>
      </footer>
    </div>
  );
};
